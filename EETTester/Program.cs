...
namespace EETTester
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            data.dat_trzby = DateTime.Now;
            data.rezim = 0;

            data.celk_trzba = 34113;
            data.cerp_zuct = 679;
            data.cest_sluz = 5460;
            data.dan1 = -172.39M;
            data.dan2 = -530.73M;
            ...
            var transformFinalBlock = sha1.TransformFinalBlock(pkp, 0, pkp.Length);
            var bkp = ByteArrayToString(transformFinalBlock);
